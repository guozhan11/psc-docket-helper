...
The `Ingest DC PSC dockets` workflow starts at 00:17, 06:17, 12:17, and 18:17 UTC. A metadata job first publishes all public PDF filing records and official links; later runs refresh the newest three days. Four extraction shards process non-overlapping record ranges one shard at a time, each downloading several PDFs in parallel. After metadata and any enabled extraction finish, one router job rebuilds the 16-part global case index. Compressed HTML, sharded case manifests, independent cursors, failed-filing lists, and the case router are stored in R2, so interrupted runs continue from their checkpoints without consuming D1's per-row daily write allowance.

Before launching any shard, the workflow checks eDocket once with the same eight-attempt backoff used by ingestion. If eDocket keeps returning a transient response (408, 429, 500, 502, 503, 504, or any Cloudflare origin error in the 520-527/530 family), the cycle is deferred: metadata, extraction, and router jobs stay skipped, the last complete R2 snapshot remains live, and the next scheduled cycle tries again. This avoids multiplying an upstream outage across four shards or rebuilding an unchanged router. The separate health monitor reports `metadata-stale` if the last successful snapshot becomes more than 36 hours old. That monitor fails the run when a condition is new or when the reported issue set changes, and stays green while an already-open alert covers exactly the same issues: a DC PSC outage in September 2026 held metadata stale for days, and a red run every half hour added nothing anyone could act on. The open issue, not the run colour, is the standing signal, and it still closes itself when health returns to `ok`.
...
The public health endpoint returns HTTP 503 with `status: "degraded"` when a metadata or ingestion shard is unavailable, when metadata or the case router is more than 36 hours old, when the router is unavailable, when a published term index is more than ten days old, or when Turnstile is disabled/misconfigured. It does not calculate a coverage percentage from partial shard state. Recent-mode metadata runs do not rescan older shards' public-PDF totals, so once every ingestion shard has reached its latest boundary, `fullTextCoverage` reconciles each shard against its own ingestion counts and reports `coverageBasis: "complete-ingestion-scan"`. Reconciliation is per shard because `documentsIndexed` is a running counter that can miss a filing when a run stops before its checkpoint, and a surplus in one shard must not hide a shortfall in another.
