...
Stored page text keeps the PDF's layout, and `findPageExcerpts` relies on it. Each page is split at blank lines into blocks, and a block is treated as a table when at least half its lines are table rows (two numeric cells, or three or more cells separated by runs of spaces) and at least one row has two numeric cells. A lone column-header line separated from its rows by a blank line joins the table, and bare page numbers are dropped. Each block is scored as a passage with BM25: a term's weight comes from how many pages of that filing contain it, repetition counts with saturation, and long blocks are normalised against the filing's average block length. A filing contributes its two best pages, ranked by their best block, and each excerpt is centred on that block (a paragraph wins a tie with a table, whose header words match many questions) and grows by whole blocks within a 2,000-character budget. Possessive query terms match the plain word, and curly apostrophes match straight ones.

Pages used to be ranked by how many distinct query terms they contained. For "what drove Pepco's 2025 O&M expense variance?", a dozen pages of Pepco's CY2025 Annual Informational Filing tied, and the cover letter won because it happened to say "Pepco's", while the O&M summary that answers the question was not selected. Under BM25 the summary ranks first. `npm run eval:retrieval` does not see this change, because it scores which filings are selected rather than which pages. Paragraphs are sent to the model as flowing text; tables keep one row per line, with cells separated by ` | ` and wrapped in `[Table]` and `[End of table]` markers.
